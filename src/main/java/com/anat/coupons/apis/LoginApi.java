...
public class LoginApi {
	...
	@PostMapping
	public UserLoginDetails login(HttpServletRequest request, HttpServletResponse response, @RequestBody UserLoginDetails userLoginDetails) throws ApplicationException {
		...
		if(userType.equals("Company")){
			...
			CompanyEntity company = this.companyController.login(userName, password);//getting the company that just logged in
			System.out.println(company);
			userLoginDetails.setUserId(company.getCompanyId());
...
//				HttpSession session = request.getSession(); // if the credentials are correct, get a session.
				...
				String userId =String.valueOf(company.getCompanyId());
...
//				request.getRequestDispatcher("http://localhost:8080/COUPONS_PROJECT_API's/companyEntrance.html").forward(request, response);
			...
			CustomerEntity customer = this.customerController.login(userName, password);
			System.out.println(customer);
			userLoginDetails.setUserId(customer.getCustomerId());
			...
			if (!customer.equals(null)){
				...
				String userId =String.valueOf(customer.getCustomerId());
				Cookie idCookie = new Cookie("userId",userId);
				response.addCookie(idCookie);	
