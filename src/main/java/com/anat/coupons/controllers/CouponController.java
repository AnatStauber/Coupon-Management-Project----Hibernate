...
//	}
	...
	public List<CouponEntity> getCouponsByCompany(long companyId) throws ApplicationException {
		...
		return this.couponDao.getCouponsByCompany(companyId);
	}
	...
	public List<PurchaseEntity> getPurchasesByCustomer(long customerId) throws ApplicationException {
		...
		validateGetPurchasesByCustomer(customerId);

		return this.couponDao.getPurchasesByCustomer(customerId);
	}
...
//			String couponImage = coupon.getCouponImage();
	...
	private void validateGetCouponsByCompany(long companyId) throws ApplicationException {
		...
		if (couponDao.getCouponsByCompany(companyId) == null) {
			throw new ApplicationException(ErrorType.EMPTY_LIST,
					"a null value was returned. seems like the list is empty." + DateUtils.getCurrentDateAndTime());
		}
	}
	...
	// list is not empty.
	private void validateGetPurchasesByCustomer(long customerId) throws ApplicationException {

		if (!this.customerDao.doesCustomerExistById(customerId)) {
			throw new ApplicationException(ErrorType.INVALID_PARAMETER,
					"customerId doesnt exist." + DateUtils.getCurrentDateAndTime());
		}
		if (couponDao.getPurchasesByCustomer(customerId) == null) {
			throw new ApplicationException(ErrorType.EMPTY_LIST,
					"a null value was returned. seems like the list is empty." + DateUtils.getCurrentDateAndTime());
		}
	}
	...
	private void validatePurchaseCoupon(long customerId, long couponId, int purchasedAmount)
		...
		CouponEntity coupon = this.couponDao.getCouponByCouponId(couponId);
